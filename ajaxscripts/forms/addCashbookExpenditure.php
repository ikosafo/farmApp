    ...
    <div class="card p-3 border-radius-xl bg-white js-active" data-animation="FadeIn" style="margin-bottom:30px">
     <h4>Add Cashbook Expenditure </h4>		
		 ...
		 <input type="hidden" id="cbUserId" name="cbUserId">
			...
			<div class="col-12 col-sm-2">
                 <label for="cbNominalAccountType">Nominal Acct (Exp Cat)</label>
                <select id="cbNominalAccountType" class="form-select" name="cbNominalAccountType" required>
                    <option value="">Select Category</option>
                    <?php
                    $getCat = $mysqli->query("select * from expense_category where ecatCode='EXP' AND ecatActive = 1");
			...
			<div class="col-12 col-sm-2">
                <label for="cbAmount">Payment Amount</label>
         ...
         <input id="cbReferenceNumber" class="form-control" type="text" name="cbReferenceNumber" placeholder="Enter reference number"> 
			...
			<div class="col-12 col-sm-2">
                  <label for="cbPaymentMode">Payment Method </label>			
		 ...
		 <div class="row mt-3">
            <div class="col-12 col-sm-4">
                <label for="cbSalesProduceType">Produce Type</label>
                 <select id="cbSalesProduceType" class="form-select" name="cbSalesProduceType" required>
                    ...
                    while ($resProd = $getProd->fetch_assoc()) { ?>
                        <option value="<?php echo $resProd['prodName'] ." Expense" ?>"><?php echo $resProd['prodName'] ." Expense" ?></option>
                    <?php } ?> 
					</select>
		
            </div>
            <div class="col-12 col-sm-4 mt-3 mt-sm-0">
               <label for="cbRecipientPayeeName">Recipient/Payer Name</label>
         <input id="cbRecipientPayeeName" class="form-control" type="text" name="cbRecipientPayeeName" placeholder="Enter Recipient name">      
		...
		<div class="col-12 col-sm-4 mt-3 mt-sm-0">
        <button  style="width: 80% !important; float: left; margin-top: 30px;" id="saveCashBookExpenditure" class="btn bg-gradient-dark mb-0 js-btn-next" type="button" title="Save Record">SAVE RECORD >> </button>    
         </div>
...
<script>
    $("#saveCashBookExpenditure").click(function() {
        var formData = {
            cbUId: $("#cbUId").val(),
        ...
        var url = "ajaxscripts/queries/addCashbookExpenditure.php";

        var successCallback = function(response) {
            console.log(response);
            if (response === 'Success') {
                $.notify("Transaction saved successfully", "success");
                loadPage("ajaxscripts/forms/cashbookExpenditure.php", function(response) {
