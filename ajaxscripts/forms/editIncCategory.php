...
$getInc = $mysqli->query("select * from `inccategory` where `icatId` = '$i_id'");
...
<form autocomplete="off" id="categoryForm">
    <div class="row g-3">
        <div class="col-12">
            ...
            placeholder="Enter category name" value="<?= $resInc['icatName']?>">
        </div>
        <div class="col-12">
            <label for="categoryDescription" class="form-label">Description</label>
            <textarea id="categoryDescription" class="form-control border-radius-md" 
            rows="3" placeholder="Enter description"><?= $resInc['icatDesc']?></textarea>
        </div>
        <?php
            $statusChecked = $resInc['icatStatus'] == 1 ? 'checked' : '';
            $statusLabel = $resInc['icatStatus'] == 1 ? 'Active' : 'Inactive';
            ?>
...
<script>
    $("#editCategory").click(function() {
        ...
        var url = "ajaxscripts/queries/editIncCategory.php";
