...
$sel = mysqli_query($mysqli, "select count(*) as allcount from general_cashbook WHERE cashBookEntryType='Income' AND cbActive = 1");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of record with filtering
$sel = mysqli_query($mysqli, "SELECT COUNT(*) AS allcount FROM general_cashbook WHERE cashBookEntryType='Income' AND cbActive = 1 AND 1 " . $searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$cashbkQuery = "SELECT * FROM general_cashbook WHERE cashBookEntryType='Income' AND cbActive = 1 AND 1 " . $searchQuery . " ORDER BY cashbkId DESC LIMIT " . $row . "," . $rowperpage;
...
while ($row = mysqli_fetch_assoc($cashbkRecords)) {
    $data[] = array(
		...
		"cbRecipientPayeeName" => mb_strimwidth($row['cbRecipientPayeeName'],0,15, '...'),	//shorten string lenght
