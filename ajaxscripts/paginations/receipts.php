...
$sel = mysqli_query($mysqli, "SELECT COUNT(*) AS allcount FROM `cashbook_transactions` WHERE `transStatus` = 1 AND `transactionType` = 'Receipt'");
if (!$sel) {
    ob_clean();
    header('Content-Type: application/json');
...
$sel = mysqli_query($mysqli, "SELECT COUNT(*) AS allcount FROM `cashbook_transactions` WHERE `transStatus` = 1 AND `transactionType` = 'Receipt' AND 1 $searchQuery");
if (!$sel) {
    ob_clean();
    header('Content-Type: application/json');
...
// Fetch records
$empQuery = "SELECT * FROM `cashbook_transactions` WHERE `transStatus` = 1 AND `transactionType` = 'Receipt' AND 1 $searchQuery ORDER BY `transactionDate` DESC, `updatedAt` DESC  LIMIT $row, $rowperpage";
